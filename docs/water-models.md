...
The SMIRNOFF format also requires definitions of some terms that are either not relevant or clearly defined for these water models, such as 1-4 scaling factors and nonbonded cutoffs. We make an effort here to choose values for these settings to ensure compatibility with the most recent mainline OpenFF force field, but if these change in the future, users may need to update these values manually to ensure force field compatibility. 
